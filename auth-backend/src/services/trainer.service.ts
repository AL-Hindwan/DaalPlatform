    ...
    async updateEnrollmentStatus(trainerId: string, enrollmentId: string, status: 'ACTIVE' | 'CANCELLED' | 'REJECT_PAYMENT' | 'REJECT_ENROLLMENT', reason?: string) {
        ...
        if (status === 'REJECT_ENROLLMENT') {
            ...
            await notificationService.createNotification({
                ...
                message: `تم رفض طلب تسجيلك في دورة "${courseTitle}". السبب: ${String(reason).trim()}. يمكنك تعديل بياناتك ثم إرسال طلب تسجيل جديد.`,
                actionUrl: `/student/courses/${enrollment.courseId}`,
                relatedEntityId: enrollmentId,
        ...
        setImmediate(async () => {
            try {
                if (isPendingMinimumCourse && targetStatus === 'PRELIMINARY_APPROVED') {
                    ...
                    const messageAction = isCancellation ? 'تم إلغاء تسجيلك في دورة' : 'نأسف، تم رفض تسجيلك في دورة';
            ...
            setImmediate(async () => {
                try {
                    ...
                    await notificationService.createNotification({
                        ...
                        title: `🔔 ${actionText} جلسة في قاعة "${session.room!.name}"`,
                        message: message,
